...
public class GeradorPlanilhaAvaliacaoVacina {
	
	public void gerarPlanilhaMediaNotaPorVacina(List<Nota> notas, String caminhoEscolhido) {
		String[] columns = { "Nome Vacina", "Média Nota" }; //teria que fazer um método no  VO vacina? 
		...
		for (Nota not : notas) {
			...
			novaLinha.createCell(0).setCellValue(not.getVacina().getNome());
			// provavemente terá de ser um DTO --> DAO 
		}
